...
class DatasetLoader:
    ...
    def get_loaders(self, augmentation=None):
        ...
        print(f"Train Data Shape: {train_data.shape}")  # (N, C, H, W)
        ...
        class CustomDataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                img = (img * 255).astype(np.uint8)  # 🔹 0~1 정규화된 값을 0~255로 변환
                img = Image.fromarray(img)  # 🔹 NumPy → PIL 변환

                if self.transform:
                    img = self.transform(img)

                img = transforms.ToTensor()(img)  # 🔹 Augmentation 후 Tensor로 변환
