...
def run_experiment(configs: omegaconf.DictConfig):
    # Set device
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    accelerator = Accelerator(gradient_accumulation_steps=configs.train_config.gradient_accumulation_steps)
    ...
    wandb.init(
        project='nlp_assignment_2',
        name=f"bert_bs{configs.data_config.batch_size}_acc{configs.train_config.gradient_accumulation_steps}",
...
def main(configs: omegaconf.DictConfig):
    accumulation_steps_list = configs.train_config.accumulation_steps
    batch_size = configs.data_config.batch_size['train']
    model_name = configs.model_config.model_name

    for accumulation_steps in accumulation_steps_list:
        print(f"\nRunning experiment with accumulation_steps: {accumulation_steps}")

        configs.train_config.gradient_accumulation_steps = accumulation_steps
        configs.train_config.checkpoint_dir = os.path.join(
            "./checkpoints", f"{model_name}-accum{accumulation_steps}"
        )
